...
const Table = (props) => {
  ...
  const renderPagination = (contentLength, pagination, pages) => {
    ...
    const goNext = (e) => { e.preventDefault(); triggerPaginate(currentPageNumber < pages.length ? currentPageNumber + 1 : pages.length) }
    const goBack = (e) => { e.preventDefault(); triggerPaginate(currentPageNumber > 0 ? currentPageNumber - 1 : 0) }

    return (
      <div className='Table-pagination'>
        <a href='#' onClick={ goBack }
          className={ `Table-pagination-prevNextButton ${ pagination.offset === 0 ? 'is-disabled' : '' }` }>
          { '\u003c\u00a0' }Back
        </a>
        <SelectInput name='go-to-page' onChange={(formId, name, val) => { triggerPaginate(val - 1) }}
          value={ currentPageNumber ? currentPageNumber + 1 : 1 } label='Go to Page:' options={ pages }
          typeAhead={ false }
        />
        <a href='#' onClick={ goNext }
          className={ `Table-pagination-prevNextButton ${ (currentPageNumber + 1) === pages.length ? 'is-disabled' : '' }` }>
          Next{ '\u00a0\u003e' }
        </a>
      </div>
    )
