...
class TableDataHandler extends React.Component {
  ...
  static propTypes = {
    ...
    query: PropTypes.shape({ /* Query is for server-side pagination, sorting, filtering, but *also* any initializing of client-side pagination, sorting, etc. */
      ...
      sort: PropTypes.shape({
        ...
        reverse: PropTypes.bool /* required if you want ascending/descending toggle behavior */
      })
    }).isRequired,
  ...
  componentDidMount() {
    ...
    if (query && !query.useServer) {
      ...
      if (data && (data.length)) {
        const offset = query.pagination && query.pagination.offset ? query.pagination.offset : 0
        const limit = query.pagination && query.pagination.limit ? query.pagination.limit : null
  ...
  render() {
    ...
    const queryData = query && query.useServer
      ? query
      : markupQuery
  ...
  return {
    sortByThisHeader: (e, header) => {
      ...
      const newSort = {
        ...
        reverse: self.state.sort.activeField === header.sortValue
      ...
      self.setState({
        activeData: newPagination.limit ? newActiveData.slice(newPagination.offset || 0, newPagination.limit) : newActiveData,
    ...
    triggerPaginate: (pageNumber) => {
      ...
      const newActiveData = self.state.pagination.limit
        ? self.sortData(self.props.data, existingSort).slice(totalOffset, self.state.pagination.limit + totalOffset)
